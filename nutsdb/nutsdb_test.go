...
import (
	...
	"github.com/philippgille/gokv/nutsdb"
...
func TestNonExistingDir(t *testing.T) {
	...
	store, err := nutsdb.NewStore(options)
...
func createStore(t *testing.T, codec encoding.Codec) (nutsdb.Store, string) {
...
func generateRandomTempDbPath(t *testing.T) string {
	path, err := ioutil.TempDir(os.TempDir(), "nutsdb")
	if err != nil {
